...
public class DashBoard extends ViewFrame {
    ...
    private Component createContent(LocalDateTime startTime) {
        ...
        FlexBoxLayout content = new FlexBoxLayout(currentTimeLabel, currentWorkingLoad, eventChannelStat, actualProcessStatData);
    ...
    private Label getCurrentTimeLabel() {
        ...
        currentTimeLabel.add(currentTime.getYear() + "." + String.format("%02d", currentTime.getMonthValue())+ "." + String.format("%02d", currentTime.getDayOfMonth()) + " " + String.format("%02d", currentTime.getHour()) + ":" + String.format("%02d", currentTime.getMinute()));
    ...
    private Component createCurrentWorkingLoadData() {
        ...
        refreshButton.addClickListener(event -> {currentTime = LocalDateTime.now(); setViewContent(createContent(startTime));});
        FlexBoxLayout headerLayout = new FlexBoxLayout(createHeader("Aktuális futási terheltség"), refreshButton);
    ...
    private Component createActualEventStat(int count) {
        ...
        switch (count) {
            case 0:
                ...
                noOfEvents.add(new Label(String.valueOf(statDataService.getNoOfAllEvents(currentTime.minusMinutes(15), currentTime))));
                dateLabel = new Label(String.format("%02d", startTime.getHour()) + ":" + String.format("%02d", startTime.getMinute()) + " - " + String.format("%02d", currentTime.getHour()) + ":" + String.format("%02d",currentTime.getMinute()));
                break;
            case 1:
                startTime = currentTime.minusHours(1);
                timeLabel = new Label("AZ ELMÚLT 1 ÓRA");
                noOfEvents.add(new Label(String.valueOf(statDataService.getNoOfAllEvents(currentTime.minusHours(1), currentTime))));
                dateLabel = new Label(String.format("%02d",startTime.getHour()) + ":" + String.format("%02d", startTime.getMinute()) + " - " + String.format("%02d",currentTime.getHour()) + ":" + String.format("%02d",currentTime.getMinute()));
                break;
            case 2:
                startTime = currentTime.minusDays(1);
                timeLabel = new Label("AZ ELMÚLT 24 ÓRA");
                noOfEvents.add(new Label(String.valueOf(statDataService.getNoOfAllEvents(currentTime.minusDays(1), currentTime))));
                dateLabel = new Label(String.format("%02d", startTime.getMonthValue()) + "." + String.format("%02d",startTime.getDayOfMonth()) + " " + String.format("%02d",startTime.getHour()) + ":" + String.format("%02d", startTime.getMinute()) +  " - " + String.format("%02d",currentTime.getMonthValue()) + "." + String.format("%02d",currentTime.getDayOfMonth()) + " " + String.format("%02d",currentTime.getHour()) + ":" + String.format("%02d",currentTime.getMinute()));
                break;
            default:
                startTime = currentTime.minusDays(7);
                timeLabel = new Label("AZ ELMÚLT HÉTEN");
                noOfEvents.add(new Label(String.valueOf(statDataService.getNoOfAllEvents(currentTime.minusDays(7), currentTime))));
                dateLabel = new Label(String.format("%02d", startTime.getMonthValue()) + "." + String.format("%02d", startTime.getDayOfMonth()) + " - " + String.format("%02d", currentTime.getMonthValue()) + "." + String.format("%02d", currentTime.getDayOfMonth()));
                break;
        ...
        actualEventStat.addClassName(count == clickedDiv ? CLASS_NAME + "__actualEvent_clicked" : CLASS_NAME + "__actualEvent");
    ...
    private Component createEventChannelStat(LocalDateTime startTime) {
        ...
        List<EventChannelStatData> eventChannelStatData = statDataService.getEventChannelStatDatas(startTime, currentTime);
        ...
        for (EventChannelStatData eventChannel : eventChannelStatData) {
            ...
            rate.add(new Label(Math.round((double)noOfSuccess * 100 / (noOfSuccess + noOfFailure)) + "%"));
            rate.add(" / ");
            rate.add(new Label(Math.round((double)noOfFailure * 100 / (noOfSuccess + noOfFailure)) + "%"));
            rateLayout.add(rate);
            averageProcessTimeLayout.add(new Label(averageProcessTime + " ms"));
        }

        eventChannelStat.add(channelLayout, successLayout, failureLayout, rateLayout, averageProcessTimeLayout);
    ...
    private Component createActualProcessStatData() {
        ...
        List<EventChannelActualProcessStatData> eventChannelActualProcessStatDatas = statDataService.getEventChannelActualProcessStatDatas(null, null);
        ...
        for (EventChannelActualProcessStatData eventChannelActualProcess : eventChannelActualProcessStatDatas) {
            ...
            for (int i = 0; i < eventChannelActualProcess.getEventData().size() && i < 3; i++) {
                ...
                noOfEvents = new Span(String.valueOf(eventChannelActualProcess.getEventData().get(i).getNoOfEvents()));
            ...
            if (eventChannelActualProcess.getEventData().size() > 3) {
                Button button = new Button(new Label("+" + (eventChannelActualProcess.getEventData().size() - 3)));
        ...
        FlexBoxLayout actualProcessStatLayout = new FlexBoxLayout(channelLayout, messageTypeLayout, noOfMessagesLayout);
